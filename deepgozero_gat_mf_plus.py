...
@ck.option(
    ...
    help='Prediction model')
@ck.option(
    '--model-name', '-m', default='deepgozero_gat_mf_plus',
    help='Prediction model')
@ck.option(
...
def main(data_root, ont, model_name, batch_size, epochs, load, device):
    ...
    terms_dict, graph, train_nids, valid_nids, test_nids, data, labels, test_df, valid_df = load_data(data_root, ont)
    ...
    nf1, nf2, nf3, nf4, relations, zero_classes = load_normal_forms(
        f'{data_root}/go-plus.norm', terms_dict)
    ...
    net = DeepGO2Model(6851, n_terms, n_rels, n_zeros, device).to(device)
...
def load_data(data_root, ont):
    ...
    return terms_dict, graph, train_nids, valid_nids, test_nids, data, labels, test_df, valid_df
...
def get_data(df, terms_dict, mfs_dict):
    ...
    for i, row in enumerate(df.itertuples()):
        for go_id in row.prop_annotations:
            if go_id in mfs_dict:
                data[i, mfs_dict[go_id]] = 1
        for go_id in row.prop_annotations:
            if go_id in terms_dict:
