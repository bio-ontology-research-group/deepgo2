...
def main(data_root, ont, batch_size, epochs, load, device):
    ...
    net = DeepGOModel(n_terms, device).to(device)
...
class DeepGOModel(nn.Module):

    def __init__(self, nb_gos, go, terms_dict, device, nb_filters=512, max_kernel=129, hidden_dim=1024):
        super().__init__()
        self.nb_gos = nb_gos
        self.go = go
        self.terms_dict = terms_dict
        self.go_matrix = self.get_go_matrix().to(device)
    ...
    def get_go_matrix(self):
        res = th.zeros((self.nb_gos, self.nb_gos), dtype=th.float32)
        for go_id, i in self.terms_dict.items():
            subs = self.go.get_term_set(go_id)
            res[i, i] = 1
            for g_id in subs:
                if g_id in self.terms_dict:
                    res[i, self.terms_dict[g_id]] = 1
        return res
    ...
    def forward(self, proteins):
        x = self.deepgocnn(proteins).unsqueeze(-1)
        x = x.expand(-1, self.nb_gos, -1)
